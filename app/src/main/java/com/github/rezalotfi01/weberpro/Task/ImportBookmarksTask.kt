...
class ImportBookmarksTask(private val fragment: SettingFragment, private val file: File) : AsyncTask<Void, Void, Boolean>() {
    ...
    override fun doInBackground(vararg params: Void): Boolean? {
        count = BrowserUtils.importBookmarks(context, file)
    ...
    override fun onPostExecute(result: Boolean?) {
        ...
        if (result!!) {
            ...
            WeberToast.show(context, context.getString(R.string.toast_import_bookmarks_successful) + count)
        } else {
            WeberToast.show(context, R.string.toast_import_bookmarks_failed)
        }
    }
}
