...
public class SwitcherPanel extends ViewGroup {
    ...
    /* parallaxOffset: dp */
    private static final int PARALLAX_OFFSET_DEFAULT_TOP = 64;
    private static final int PARALLAX_OFFSET_DEFAULT_BOTTOM = 16;
    private int parallaxOffset = PARALLAX_OFFSET_DEFAULT_TOP;

    /* flingVelocity: dp/s */
    private static final int FLING_VELOCITY_DEFAULT = 256;

    private void setFlingVelocity(int flingVelocity) {
        int flingVelocity1 = flingVelocity;
        if (dragHelper != null) {
            dragHelper.setMinVelocity(ViewUtils.INSTANCE.dp2px(getContext(), flingVelocity));
        }
    }
    ...
    public SwitcherPanel(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
            ...
            if (ac == 0) {
                ...
                shadowDrawable = ViewUtils.INSTANCE.getDrawable(getContext(), R.drawable.shadow_below);
            } else {
                anchor = Anchor.BOTTOM;
                parallaxOffset = PARALLAX_OFFSET_DEFAULT_BOTTOM;
                shadowDrawable = ViewUtils.INSTANCE.getDrawable(getContext(), R.drawable.shadow_above);
            }
        ...
        float dimen108dp = getResources().getDimensionPixelSize(R.dimen.layout_height_108dp);
            ...
            int windowHeight = ViewUtils.INSTANCE.getWindowHeight(context);
            int statusBarHeight = ViewUtils.INSTANCE.getStatusBarHeight(context);
...
//            throw new IllegalStateException("Width must have an exact value or MATCH_PARENT.");
    ...
    public void draw(Canvas canvas) {
        ...
        int shadowHeight = SHADOW_HEIGHT_DEFAULT;
        if (anchor == Anchor.TOP) {
            top = (int) (mainView.getTop() + dimen48dp);
            bottom = top + ((int) (ViewUtils.INSTANCE.dp2px(getContext(), shadowHeight)));
        } else {
            bottom = (int) (mainView.getBottom() - dimen48dp);
            top = bottom - ((int) (ViewUtils.INSTANCE.dp2px(getContext(), shadowHeight)));
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (!keyBoardShowing && omnibox.getVisibility() == VISIBLE && shouldCollapsed()) {
                ...
                if (anchor == Anchor.TOP && deltaY >= ViewUtils.INSTANCE.dp2px(getContext(), 32)) {
                    collapsed();
                    return true;
                } else if (anchor == Anchor.BOTTOM && deltaY <= -ViewUtils.INSTANCE.dp2px(getContext(), 32)) {
                    collapsed();
                    return true;
                }
            }
    ...
    private void applyParallaxForCurrentSlideOffset() {
        if (parallaxOffset > 0) {
            float offset = ViewUtils.INSTANCE.dp2px(getContext(), parallaxOffset);
            if (anchor == Anchor.TOP) {
