...
public class ReadabilityActivity extends WeberActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        progressBar = findViewById(R.id.readability_progress);
        webView = findViewById(R.id.readability_webview);
        emptyView = findViewById(R.id.readability_empty);
        showLoadStart();
        ...
        if (intent == null || !intent.hasExtra(IntentUtils.URL) || token == null || token.trim().isEmpty()) {
            showLoadError();
        } else {
            String url = intent.getStringExtra(IntentUtils.URL);
            String query = REQUEST.replace(REQUEST_URL, url).replace(REQUEST_TOKEN, token);
    ...
    public void showLoadSuccessful() {
        try {
            ...
            webView.loadDataWithBaseURL(BrowserUtils.BASE_URL, content, BrowserUtils.MIME_TYPE_TEXT_HTML, BrowserUtils.URL_ENCODING, null);
