...
class ExportBookmarksTask(private val context: Context) : AsyncTask<Void, Void, Boolean>() {
    ...
    override fun doInBackground(vararg params: Void): Boolean? {
        path = BrowserUtils.exportBookmarks(context)
    ...
    override fun onPostExecute(result: Boolean?) {
        ...
        if (result!!) {
            WeberToast.show(context, context.getString(R.string.toast_export_bookmarks_successful) + path!!)
        } else {
            WeberToast.show(context, R.string.toast_export_bookmarks_failed)
        }
    }
}
